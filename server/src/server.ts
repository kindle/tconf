...
function getSymbolAtPosition(position: Position, documentUri: string): string {
    const range = {
...
DocData.metaData.forEach((m:any)=>{
	let doc = `
Name: ${m.name}
...
Signature: ${m.functionSignature}

`;
m.parameters.forEach((p:any)=>{
	p.split('\n').forEach((pp:any,index:number)=>{
		doc+=(index>0?`    `:`  •  `)+pp;
